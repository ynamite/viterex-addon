...
- `DeployFile::rewrite(string $contents, ?array $extracted): string` — produces new file contents. If markers are present, replaces the region between them. Otherwise, replaces the **prologue plus the contiguous span containing all recognized `host(...)` chains** with the marker region. (For multi-host files, this means a single span from the first prologue assignment through the closing `;` of the last `host()->...` chain — anything between/after is preserved verbatim. Implementation note: the simplest approach is to find the byte range from the first prologue token to the last token of the final recognized host chain and replace that whole range.) The injected block is generated from a single fixed template; it always reads from the sidecar via `$cfg`.
